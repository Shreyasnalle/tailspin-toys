...
The repository's coding standards are documented in the [Copilot instructions](.github/copilot-instructions.md) and the scoped files in [.github/instructions](.github/instructions/). In particular:

- Comments explain intent, rationale, and non-obvious constraints; they do not restate code and must be kept current.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc to document their purpose, parameters, and return values, including injectable database parameters.
- Reusable Astro components document their `Props` interfaces and non-obvious prop constraints.
